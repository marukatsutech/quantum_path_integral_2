# Quantum Mechanics, Path integral - 2_2
...
def step():
    ...
    qtm1 = qtm1_buffer / np.sum(np.abs(qtm1_buffer)) * 2.
    x1 = x
    y1 = qtm1.imag
    z1 = qtm1.real
    y1_offset = y1 + offset
...
def update_qtm1():
    ...
    if var_function.get() == 0:
        qtm1_buffer = np.dot(qtm0a + qtm0b, phase_convert_array)
    else:
        qtm1_buffer = np.dot(qtm0a - qtm0b, phase_convert_array)
    qtm1 = qtm1_buffer / np.sum(np.abs(qtm1_buffer)) * 2.
    x1 = x
...
def update_qtm0b():
    ...
    z0b = np.sin(k0b * x * k_x_ratio) * gaussian0b_std
    y0b = np.cos(k0b * x * k_x_ratio) * 1j * gaussian0b_std
    qtm0b = z0b + y0b
    yb = qtm0b.imag
    zb = qtm0b.real
    yb_offset = yb - offset
...
var_k0b.set(str(k0b))
spn_k0b = tk.Spinbox(
    frm_qtm_b, textvariable=var_k0b, format="%.1f", from_=-20., to=20., increment=0.5,
...
spn_s0b = tk.Spinbox(
    frm_qtm_b, textvariable=var_s0b, format="%.1f", from_=-400, to=400, increment=10,
...
rdb0_function = tk.Radiobutton(frm_fn, text="Qtm dt=QtmA+QtmB", command=lambda: change_function(),
                               variable=var_function, value=0)
rdb1_function = tk.Radiobutton(frm_fn, text="Qtm dt=QtmA-QtmB", command=lambda: change_function(),
